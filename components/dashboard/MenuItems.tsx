  ...
  const menuItems = [
    {
      ...
      items: [
        ...
        {
          id:`2-general`,
          link: admin && user.user_id? `/dashboard/orders/${user?.user_id}` :`/dashboard/projects/${user?.user_id}`,
          icon: <Wrench />,
            ...
            <React.Fragment key={`menu-${index}`}>
              <CommandGroup key={`group-${index}`} heading={menu.group} className="text-xl">
                {menu.items.map((item, itemIndex) => (
                  <Link href={`${item.link}`} key={`link-${itemIndex}`} onClick={()=>setActiveMenuItemId(item.id)} className="mt-4">
                    <button onClick={()=>toggleMenu()}>
                    <CommandItem key={`item-${itemIndex}`} className={cn("cursor-pointer gap-2 flex",activeMenuItemId === item.id ? "bg-black text-white":" hover:text-black hover:bg-opacity-15 hover:bg-black")}>
                      {item.icon}
                      {item.text}
                    </CommandItem>
                    </button>
